...
const ComplianceModal = ({ isOpen, onClose, policy }) => {
  const { addPolicy, updatePolicy, showToast } = useAdmin();
             ...
             <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto flex flex-col h-full text-left">
                <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/20">
                   <div className="flex items-center gap-5">
                      ...
                      <div>
                         <h2 className="text-xl font-extrabold text-slate-900 dark:text-white leading-none">{policy ? 'Edit Policy' : 'Publish Policy'}</h2>
                   ...
                   <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-2 gap-8">
                         ...
                         <div className="space-y-2">
                            ...
                            <input type="date" value={formData.effectiveDate} onChange={e => setFormData({...formData, effectiveDate: e.target.value})} className="input-field h-14 bg-slate-50 dark:bg-slate-850 border-transparent font-bold text-slate-700 dark:text-slate-200" />
                         </div>
                      </div>
