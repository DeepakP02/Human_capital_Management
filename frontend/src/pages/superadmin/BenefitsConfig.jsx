...
const CreatePlanModal = ({ onClose, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onSave({ ...form, id: `BP-${Date.now()}`, monthlyCost: parseFloat(form.monthlyCost) || 0 });
    toast(`Benefit plan "${form.name}" created successfully`);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-base font-black text-slate-800 dark:text-slate-100">Create Benefit Plan</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Monthly Cost ($/employee) *</label>
...
const BenefitsConfig = () => {
  ...
  // ── Analytics trends (static representative data) ─────────────────────────
  const enrollTrend  = [65, 72, 78, 75, 82, 88, 86, 91, 89, 93, 90, 95];
  const usageTrend   = [45, 52, 58, 55, 63, 68, 71, 74, 70, 76, 79, 82];
  const costTrend    = [18, 20, 22, 21, 24, 26, 25, 28, 27, 30, 29, 32];
  ...
  const handleArchive = (id) => {
    setBenefitPlans(prev => prev.map(p => p.id === id ? { ...p, status: 'Archived' } : p));
    toast('Plan archived successfully');
  };
    ...
    >
      ...
      <motion.div variants={itemVariants}>
        <PageHeader
          ...
          subtitle="Super Admin • Configure plans, manage enrollment & track utilization"
        >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        />
        <KPICard
          icon={DollarSign} label="Monthly Benefits Cost" value={`$${metrics.monthlyCost.toLocaleString()}`}
      ...
      {!hasPlans && (
        <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-10 text-center shadow-sm">
          ...
          <h3 className="text-base font-bold text-slate-700 dark:text-slate-300 mb-1">No benefit plans created yet</h3>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            <button className="btn-secondary flex items-center gap-2 text-xs" onClick={() => toast('Import plans', 'info')}><Upload size={14}/>Import Plans</button>
            <button className="btn-secondary flex items-center gap-2 text-xs" onClick={() => toast('Configure benefits', 'info')}><Settings size={14}/>Configure Benefits</button>
          </div>
        </motion.div>
      )}
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800/60">
                      {filteredPlans.map((plan, idx) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-xs font-bold text-slate-700 dark:text-slate-300">${(plan.monthlyCost || 0).toLocaleString()}</td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <button onClick={() => toast(`Viewing ${plan.name}`, 'info')} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-primary-600 transition-colors" title="View">
                                <Eye size={13} />
                              </button>
                              <button onClick={() => { setEditingPlan(plan); toast('Edit mode', 'info'); }} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-amber-600 transition-colors" title="Edit">
                                <Edit2 size={13} />
                              </button>
                              <button onClick={() => handleArchive(plan.id)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-rose-600 transition-colors" title="Archive">
                                <Archive size={13} />
                              </button>
                            </div>
                          </td>
                  ...
                  {filteredPlans.map(plan => (
                    <div key={plan.id} className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-500 mt-3">
                        <div>Cost: <span className="font-bold text-slate-700 dark:text-slate-300">${plan.monthlyCost || 0}/mo</span></div>
                        <div>Enrolled: <span className="font-bold text-slate-700 dark:text-slate-300">{plan.enrolledCount || 0}</span></div>
                      </div>
                      <div className="flex gap-2 mt-3">
                        <button onClick={() => toast(`View ${plan.name}`, 'info')} className="flex-1 text-[10px] font-bold text-primary-600 bg-primary-50 dark:bg-primary-950/20 py-1.5 rounded-lg">View</button>
                        <button onClick={() => toast(`Edit ${plan.name}`, 'info')} className="flex-1 text-[10px] font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/20 py-1.5 rounded-lg">Edit</button>
                        <button onClick={() => handleArchive(plan.id)} className="flex-1 text-[10px] font-bold text-rose-600 bg-rose-50 dark:bg-rose-950/20 py-1.5 rounded-lg">Archive</button>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div key={i} className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-primary-200 dark:hover:border-primary-800 transition-colors">
                    <div className="hidden sm:grid grid-cols-5 gap-3 items-center">
                      ...
                      <div className="col-span-2">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-[10px] font-bold text-slate-400 whitespace-nowrap">${dept.cost.toLocaleString()}/mo</span>
                        </div>
                      </div>
                    </div>
                    {/* Mobile */}
                    <div className="block sm:hidden">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-[10px] text-slate-400">
                        ...
                        <span>Cost: <strong className="text-slate-600 dark:text-slate-300">${dept.cost}/mo</strong></span>
                      </div>
                    </div>
                  </div>
                ))}
