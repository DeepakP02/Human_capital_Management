  ...
  useEffect(() => {
    ...
    fetchAttendanceLogs(employeeId)
      .then((logs) => {
        if (logs.length === 0) {
          const mock = [
            { date: new Date().toISOString().slice(0, 10), timeIn: '09:00', timeOut: '17:00', status: 'OnTime' },
            { date: new Date().toISOString().slice(0, 10), timeIn: '09:30', timeOut: '17:30', status: 'Late' },
          ];
          setAttendanceLogs(mock);
        } else {
