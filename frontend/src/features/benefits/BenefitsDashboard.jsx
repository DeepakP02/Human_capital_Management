...
const BenefitsDashboard = () => {
  ...
  const [wellnessCampaigns, setWellnessCampaigns] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: 1, title: 'Summer Wellness Challenge', description: 'Run or walk 50k this month to earn health premiums deduction.', participants: 84, target: 100, budget: '$4,500' },
      { id: 2, title: 'Mindfulness & Meditation', description: 'Access to premium Headspace subscriptions for mental wellness.', participants: 120, target: 150, budget: '$2,400' },
      { id: 3, title: 'Smart Eating Subsidies', description: '50% off gym cafe salads and nutritional meals.', participants: 42, target: 60, budget: '$1,800' }
    ];
  });
  ...
  const loadGlobalData = () => {
    ...
    if (ben) {
      ...
      const defaults = [
        { id: 1, name: 'Platinum Health Plus', category: 'Insurance', provider: 'Global Health Inc.', contribution: '$450/m', eligibility: 'Full-time Only', status: 'Active' },
        { id: 2, name: 'Mental Wellness Sub', category: 'Wellness', provider: 'MindScale', contribution: '$25/m', eligibility: 'All Employees', status: 'Active' },
        { id: 3, name: 'Learning & Dev Fund', category: 'Reimbursement', provider: 'Self-Funded', contribution: 'Up to $2k/y', eligibility: 'Full-time Only', status: 'Active' },
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50 text-xs">
                  {filteredClaims.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/10 transition-colors">
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <img src={`https://i.pravatar.cc/150?u=${c.name}`} alt={c.name} className="w-9 h-9 rounded-xl object-cover" />
                          <div>
