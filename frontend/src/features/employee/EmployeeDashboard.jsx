...
const EmployeeDashboard = () => {
  ...
  const stats = [
    { label: 'Today Attendance', value: attendance.isClockedIn ? 'Clocked In' : 'Not In', trend: attendance.isClockedIn ? `Since ${new Date(attendance.clockInTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'Ready to start?', icon: Clock, color: attendance.isClockedIn ? 'text-emerald-600 dark:text-emerald-450' : 'text-slate-400 dark:text-slate-500', bg: attendance.isClockedIn ? 'bg-emerald-50 dark:bg-emerald-950/20' : 'bg-slate-100 dark:bg-slate-800/40' },
    { label: 'Pending Leaves', value: leaves.requests.filter(r => r.status === 'Pending').length, trend: 'Awaiting approval', icon: CalendarDays, color: 'text-amber-600 dark:text-amber-455', bg: 'bg-amber-50 dark:bg-amber-950/20' },
    { label: 'Salary Status', value: 'Paid', trend: 'Credited on 31st Oct', icon: Wallet, color: 'text-indigo-600 dark:text-indigo-400', bg: 'bg-indigo-50 dark:bg-indigo-950/20' },
    { label: 'Active Goals', value: performance.goals.length, trend: `${performance.goals.filter(g => g.progress === 100).length} completed`, icon: CheckSquare, color: 'text-primary-600 dark:text-primary-400', bg: 'bg-primary-50 dark:bg-primary-950/20' },
  ];
  ...
  return (
    <div className="space-y-8 pb-12 animate-fade-in focus:outline-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="hcm-page-title">Welcome Back, {profile.fullName.split(' ')[0]}!</h1>
          <p className="hcm-page-subtitle text-lg">Everything looks great. You have {performance.goals.filter(g=>g.progress < 100).length} active goals to focus on.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-8 w-full border-t md:border-t-0 md:border-l border-slate-100 dark:border-slate-800 md:pl-10 pt-8 md:pt-0 text-left">
                 <div className="space-y-6">
                    ...
                    <div className="space-y-1">
                       ...
                       <p className="text-lg font-black text-slate-850 dark:text-slate-200">Office-Based</p>
                    </div>
                 </div>
                 <div className="space-y-6">
                    ...
                    <div key={goal.id} className="p-6 flex items-center justify-between group hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors">
                       <div className="flex-1 mr-8 text-left">
                          <div className="flex items-center justify-between mb-2">
                             <span className="text-sm font-black text-slate-800 dark:text-slate-200 uppercase tracking-tight">{goal.title}</span>
        ...
        <div className="space-y-8 h-full">
           ...
           {/* Announcements Panel */}
            ...
            <div className="card p-8 group hover:shadow-xl transition-all text-left">
               ...
               <div className="flex items-center gap-5">
                  <div className="w-16 h-16 rounded-[1.5rem] bg-indigo-50 dark:bg-indigo-950/20 border-2 border-indigo-100 dark:border-indigo-900/30 flex flex-col items-center justify-center shadow-inner">
                     <span className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">Oct</span>
                     <span className="text-2xl font-black text-slate-900 dark:text-white leading-none mt-1">31</span>
                  </div>
                  <div>
                     <p className="text-base font-black text-slate-900 dark:text-white leading-none">Halloween Fest</p>
                     <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1.5">Optional Holiday</p>
                  </div>
               </div>
               <div className="mt-8 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                     <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                     <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Next Friday</span>
                  </div>
      ...
      <CenterModal isOpen={showLeaveModal} onClose={() => !isSubmittingLeave && setShowLeaveModal(false)} title="Request New Leave">
         <form onSubmit={handleRequestLeave} className="p-8 space-y-6 bg-white dark:bg-slate-900">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-left">
               <div className="space-y-2 text-left">
                  <label className="form-label px-1">Leave Type</label>
                  <select name="type" disabled={isSubmittingLeave} className="input-field h-12 font-semibold">
               ...
               <div className="space-y-2 text-left">
                  <label className="form-label px-1">Attachment (Optional)</label>
                  <div className="h-12 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl flex items-center justify-center text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest bg-slate-50 dark:bg-slate-900 cursor-pointer hover:border-primary-400 transition-colors">
                     Upload File
                  </div>
               </div>
               <div className="space-y-2 text-left">
                  <label className="form-label px-1">Start Date</label>
                  <input name="startDate" type="date" required disabled={isSubmittingLeave} className="input-field h-12 font-semibold" />
               </div>
               <div className="space-y-2 text-left">
                  <label className="form-label px-1">End Date</label>
                  <input name="endDate" type="date" required disabled={isSubmittingLeave} className="input-field h-12 font-semibold" />
               </div>
            </div>
            <div className="space-y-2 text-left">
               <label className="form-label px-1">Reason for Leave</label>
               <textarea name="reason" rows="3" required disabled={isSubmittingLeave} className="input-field py-4 resize-none font-semibold" placeholder="Provide a brief explanation..."></textarea>
            </div>
            <div className="space-y-2 text-left">
               <label className="form-label px-1">Emergency Contact While Away</label>
               <input name="emergency" type="text" required disabled={isSubmittingLeave} placeholder="+1 (555) 000-0000" className="input-field h-12 font-semibold" />
            </div>
            <div className="pt-4 flex gap-4 text-left">
               <button type="button" disabled={isSubmittingLeave} onClick={() => setShowLeaveModal(false)} className="btn-secondary flex-1 py-3 text-sm">Cancel</button>
               <button type="submit" disabled={isSubmittingLeave} className="btn-primary flex-2 py-3 text-sm flex items-center justify-center gap-2">
                  {isSubmittingLeave ? (
                    <>
                      <Loader2 size={18} className="animate-spin" />
                      <span>{leaveStep}</span>
      ...
      <CenterModal isOpen={!!selectedAnnouncement} onClose={() => setSelectedAnnouncement(null)} title="Announcement Details">
         {selectedAnnouncement && (
            <div className="p-8 text-left">
               ...
               <h2 className="text-2xl font-black text-slate-900 mb-4 dark:text-white">{selectedAnnouncement.title}</h2>
            ...
            <div className="space-y-6">
               {announcements.map((ann, i) => (
                  <div key={i} className="p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:border-primary-200 transition-all cursor-pointer" onClick={() => { setShowDetailBoardModal(false); setSelectedAnnouncement(ann); }}>
                     ...
                     <h4 className="text-base font-black text-slate-900 leading-tight mb-2 dark:text-white">{ann.title}</h4>
      ...
      <CenterModal isOpen={showHolidayModal} onClose={() => setShowHolidayModal(false)} title="Halloween Fest Holiday">
         <div className="p-8 text-left space-y-6">
            <div className="flex items-center gap-5 p-6 bg-slate-50 border border-slate-100 rounded-3xl">
               ...
               <div>
                  <h3 className="text-lg font-black text-slate-900 leading-none dark:text-white">Halloween Fest</h3>
