    ...
    setTimeout(() => {
      try {
        if (selectedFormat === 'PDF Report') {
          ...
          link.setAttribute("download", `Team_Report_${dateRange.replace(/\s+/g, '_')}.${selectedFormat === 'Excel Sheet' ? 'xlsx' : 'csv'}`);
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
          showToast(`Downloaded ${selectedFormat} file successfully!`, 'success');
        } else {
          let pptText = `HCM.ai Presentation Outline - Team Performance\n`;
          pptText += `==============================================\n`;
          pptText += `Slide 1: Team Report Summary\n`;
          pptText += `   - Reporting Range: ${dateRange}\n`;
          pptText += `   - Active Team Roster Size: ${teamMembers.length} Employees\n\n`;
          pptText += `Slide 2: Active Goals & Objectives\n`;
          kpis.slice(0, 3).forEach((k, idx) => {
            pptText += `   - Goal ${idx+1}: ${k.title} (${k.progress}% Complete - ${k.status})\n`;
          });
          pptText += `\nSlide 3: Tasks and Deliverables\n`;
          tasks.slice(0, 3).forEach((t, idx) => {
            pptText += `   - Task ${idx+1}: ${t.title} (Assignee: ${t.assignee} - Due: ${t.dueDate || 'N/A'})\n`;
          });
          
          const element = document.createElement("a");
          const file = new Blob([pptText], {type: 'text/plain'});
          element.href = URL.createObjectURL(file);
          element.download = `Team_Presentation_Outline_${dateRange.replace(/\s+/g, '_')}.txt`;
          document.body.appendChild(element);
          element.click();
          document.body.removeChild(element);
          showToast(`Downloaded PowerPoint report outline successfully!`, 'success');
        }
      } catch (err) {
        console.error(err);
